...
# col = []
# for i in range(256): # x5=x1x2x3
#     col.append(matrix[1][i] * matrix[2][i] * matrix[3][i])
# matrix.append(col)

# col = []
# for i in range(256): # x6=x1x2x4
#     col.append(matrix[1][i] * matrix[2][i] * matrix[4][i])
# matrix.append(col)

# col = []
# for i in range(256): # x7=x1x3x4
#     col.append(matrix[1][i] * matrix[3][i] * matrix[4][i])
# matrix.append(col)

# col = []
# for i in range(256): # x8=x2x3x4
#     col.append(matrix[2][i] * matrix[3][i] * matrix[4][i])
# matrix.append(col)

col = []
for i in range(256): # x5=x1x2
    col.append(matrix[1][i] * matrix[2][i])
matrix.append(col)

col = []
for i in range(256): # x6=x1x3
    col.append(matrix[1][i] * matrix[3][i])
matrix.append(col)

col = []
for i in range(256): # x7=x1x4
    col.append(matrix[1][i] * matrix[4][i])
matrix.append(col)

col = []
for i in range(256): # x8=x2x4
    col.append(matrix[2][i] * matrix[4][i])
matrix.append(col)
...
with open('ffd16.csv', 'w', encoding='utf-16') as f:
    line = 'N,'
    line += ','.join(col_names[:16])
