...
col = []
for i in range(256): # x0
    col.append(1)
matrix.append(col)

col = []
for i in range(128): # x1
    col.append(-1)
    col.append(1)
matrix.append(col)

col = []
for i in range(64): # x2
    col.append(-1)
    col.append(-1)
    col.append(1)
    col.append(1)
...
with open('ffd.csv', 'w', encoding='utf-16') as f:
    line = 'N,'
    line += ','.join(col_names[:FFD_N])
    f.write(f'{line},Y ф.,Y лин.,Y ф. - Y лин.,Y ч.-н.,Y ф. - Y ч.-н.\n')

    for i in range(FFD_N):
        line = f'{i + 1},'
        for j in range(FFD_N):
            line += f'{matrix[j][i]},'
        f.write(f'{line}\n')
